...
from agentic_fs.models.admin import NamespaceListResponse

router = APIRouter()


@router.get("/v1/{tenant}/namespaces", response_model=NamespaceListResponse)
async def list_namespaces(tenant: str):
    """List all namespaces within a tenant.

    Scans the tenant's namespace directory for all existing namespaces.
    Useful for discovering what namespace groupings exist before browsing
    directories or searching.
    """
    fs = get_file_store()
    namespaces = fs.list_namespaces(tenant)
...
@router.get("/v1/{tenant}/dirs", response_model=DirListResponse)
@router.get("/v1/{tenant}/dirs/{path:path}", response_model=DirListResponse)
async def list_directory(
    tenant: str,
    path: str = "",
    namespace: str = Query(default="default", description="Namespace to browse."),
    recursive: bool = Query(default=False, description="If true, recurse into all subdirectories and return a flat list of all entries."),
):
    ...
    Pass an empty path (or omit the path segment) to list the root of the namespace.
    Subdirectory entries can be traversed by appending their `name` to the path.

    Set `recursive=true` to get a flat list of all files and directories in the
    entire subtree.
    """
    fs = get_file_store()
    if recursive:
        entries = fs.list_directory_recursive(tenant, namespace=namespace, path=path)
    else:
